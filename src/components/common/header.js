...
<header id="Header">
    ...
    <body>
        ...
        <script>
            // Chèn headerHTML vào DOM
            document.getElementById("header-container").innerHTML = headerHTML;

            // Gắn ảnh đại diện sau khi DOM sẵn sàng
            setTimeout(() => {
                const loggedUser = JSON.parse(localStorage.getItem("loggedUser"));
                const avatar = document.getElementById('anhdaidien');
                if (loggedUser && loggedUser.user && loggedUser.user.anhDaiDien) {
                    avatar.setAttribute('src', loggedUser.user.anhDaiDien);
                } else {
                    console.error("Dữ liệu trong localStorage không hợp lệ hoặc thiếu ảnh đại diện.");
                }
            }, 0);
        </script>
    </body>
